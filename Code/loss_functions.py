...
def photometric_reconstruction_loss(tgt_img, ref_imgs, intrinsics,
    ...
    #print(pose[:,:,3:].shape)
    #print(transforms.euler_angles_to_matrix( pose[:,:,3:],[ "X","Y","Z"]))
    # R=transforms.euler_angles_to_matrix( pose[:,:,3:],[ "X","Y","Z"])
    # print(torch.stack(depth))
    # for d in depth:
    #     print(d.shape)
    # print(tgt_img.shape)
    # print(torch.stack(depth).shape)
    # print(pose[0])
    # print(R[:,0,2,:].shape)
    # pdb.set_trace()
    # error_r_depth=torch.matmul(R[:,:,2],torch.stack(depth))
    # print(error_r_depth)
    def one_scale(depth, explainability_mask):
        ...
        for i, ref_img in enumerate(ref_imgs_scaled):
            ...
            ref_img_warped, valid_points,depth_constr = inverse_warp(ref_img, depth[:,0], current_pose,
                                                        intrinsics_scaled,
                                                        rotation_mode, padding_mode)
            dp_const=nn.ReLU()(depth_constr)
            epsilon = 0.0000002
            dp_const_count=torch.square(dp_const)/(torch.square(dp_const)+epsilon)
        ...
        return reconstruction_loss, warped_imgs, diff_maps,1/dp_const_count
    ...
    for d, mask in zip(depth, explainability_mask):
        # print(d.shape)
        loss, warped, diff,depth_cons = one_scale(d, mask)
        total_loss += loss
        # print("losss",total_loss)
        # print("lossfuck",torch.mean(depth_cons))
        t_depth_cons+=torch.mean(depth_cons)
        warped_results.append(warped)
        diff_results.append(diff)
        # depth_const.append(dp_con)
    # depth_const=depth_const/tgt_img.shape[0]
    # print('dc',t_depth_cons)
    return total_loss, warped_results, diff_results,t_depth_cons
def chierality(tgt_img, ref_imgs, intrinsics,
                                    depth, explainability_mask, pose,
                                    rotation_mode='euler', padding_mode='zeros'):
